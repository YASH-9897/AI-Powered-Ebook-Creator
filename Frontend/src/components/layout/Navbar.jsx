        ...
        <nav className="hidden lg:flex items-center space-x-1">
          {navlinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="px-4 py-2 text-m font-medium text-gray-600 hover:text-violet-600 rounded-lg hover:bg-violet-50/50 transition-all duration-200"
            >
              {link.name}
            </a>
          ))}
      ...
      <div className="lg:hidden bg-white border-t border-gray-100 animate-in slide-in-from-top duration-200">
        <nav className="px-4 py-4 space-y-1">
          ...
          <a
            key={link.name}
            href={link.href}
            className="block px-4 py-2.5 rounded-lg text-sm font-mdium text-gray-700 hover:text-violet-600 hover:bg-violet-50 transition-all duration-200"
          >
            {link.name}
          </a>
))}
        ...
        <div className="px-4 py-4 border-t border-gray-100">
          {isAuthenticated ? (
            ...
            <div className="">
              ...
              <a
                href="/signup"
                className="block text-center px-4 py-2.5 text-sm font-medium text-white bg-linear-to-r from-violet-600 to-purple-600 rounded-lg shaow-lg shadow-iolet-500/30  hover:text-gray-900   transition-all duration-200"
