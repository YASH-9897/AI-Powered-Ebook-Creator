...
//process markdown content into docx paragrapgh
...
const exportAsPDF = async (req, res) => {
    try {
        ...
        //cover page with image if availlable
        if (book.coverImage && !book.coverImage.includes("pravatar")) {
            const imagePath = book.coverImage.substring(1);
            ...
            try {
                ...
                console.error(`Could not mbed image: ${imagePath}`, imgErr);
            }
        }
        ...
        if (!res.headersSent) {
            res.status(500).json({
                message: "Server error during exporting PDF",
                error: error.message,
            });
        }
    }
